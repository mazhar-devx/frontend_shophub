    ...
    return (
      <div className="min-h-screen pt-24 px-4 flex justify-center items-center relative overflow-hidden">
         ...
         <div className="relative">
             <div className="w-16 h-16 border-4 border-black/5 dark:border-white/10 border-t-purple-500 rounded-full animate-spin"></div>
    ...
    <div className="min-h-screen pt-24 pb-20 px-4 sm:px-6 relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto relative z-10">
        ...
        <div className="text-center mb-16 space-y-4">
            ...
            <p className="text-gray-600 dark:text-gray-500 dark:text-gray-400 text-lg md:text-xl max-w-2xl mx-auto leading-relaxed animate-fade-in-up delay-100">
                Dive into our curated categories. From latest tech to fashion trends, find exactly what you're looking for.
            </p>
            
            {/* Search & Filter Bar */}
            <div className="max-w-2xl mx-auto mt-10 p-2 bg-black/5 dark:bg-white/5 backdrop-blur-xl border border-black/5 dark:border-white/10 rounded-2xl flex flex-col sm:flex-row gap-2 shadow-2xl animate-fade-in-up delay-200">
                <div className="relative flex-grow">
                    <svg className="absolute left-4 top-1/2 -translate-y-1/2 w-5 h-5 text-gray-600 dark:text-gray-500 dark:text-gray-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    ...
                    <input 
                        ...
                        className="w-full bg-transparent border-none text-primary dark:text-white pl-12 pr-4 py-3 focus:ring-0 placeholder-gray-500"
                    />
                </div>
                <div className="flex bg-black/20 rounded-xl p-1">
                    {['All', 'Popular'].map((filter) => (
                        <button
                            ...
                            className={`px-6 py-2 rounded-lg text-sm font-bold transition-all ${activeFilter === filter ? 'bg-black/10 dark:bg-white/10 text-primary dark:text-white shadow-lg' : 'text-gray-600 dark:text-gray-500 dark:text-gray-400 hover:text-primary dark:text-white'}`}
                        >
                            {filter}
                        </button>
                    ))}
          ...
          {filteredCategories.map((cat, index) => {
             ...
             return (
                ...
                >
                  ...
                  <div className="glass h-full border border-white/5 hover:border-black/10 dark:border-white/20 rounded-3xl p-6 flex flex-col items-center text-center transition-all duration-300 transform group-hover:-translate-y-2 bg-white dark:bg-[#0a0a0a]/40 backdrop-blur-md relative overflow-hidden">
                      ...
                      <div className="mb-4 relative">
                          ...
                          {count > 0 && (
                              <span className="absolute -top-2 -right-4 bg-gradient-to-r from-purple-600 to-cyan-500 text-primary dark:text-white text-[10px] font-bold px-2 py-0.5 rounded-full shadow-lg border border-black animate-pulse">
                                  {count}
                              </span>
                          )}
                      </div>
                      
                      <h3 className="text-primary dark:text-white font-bold text-lg mb-1 group-hover:text-transparent group-hover:bg-clip-text group-hover:bg-gradient-to-r group-hover:from-cyan-300 group-hover:to-purple-300 transition-all">
                          {cat.name}
                      </h3>
        ...
        {filteredCategories.length === 0 && (
            <div className="text-center py-20">
                ...
                <p className="text-gray-600 dark:text-gray-500 dark:text-gray-400">Try adjusting your search terms.</p>
            </div>
        )}
      </div>
    </div>
