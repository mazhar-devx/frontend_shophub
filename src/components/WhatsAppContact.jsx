...
const WhatsAppContact = () => {
    ...
    return (
        <div className="fixed max-md:bottom-24 bottom-6 right-6 z-[999] flex flex-col items-end pointer-events-none group/wa">
            {/* Modal Overlay */}
            <div className={`mb-4 w-[280px] sm:w-[350px] pointer-events-auto transition-all duration-500 transform ${isOpen ? 'translate-y-0 opacity-100 scale-100' : 'translate-y-10 opacity-0 scale-95 pointer-events-none'}`}>
                <div className="glass rounded-[2.5rem] border border-white/20 shadow-xl overflow-hidden relative md:backdrop-blur-3xl">
                    {/* Background Glow */}
                        ...
                        <div className="space-y-3">
                            <div className="flex items-center justify-between px-1">
                             ...
                             <div className="grid grid-cols-2 gap-2">
                                <div className="bg-black/5 dark:bg-black/30 border border-black/5 dark:border-white/5 rounded-xl p-3 flex flex-col items-center gap-1 hover:border-purple-500/30 transition-all cursor-default group/item">
                                    <span className="text-[10px] font-black text-purple-600 dark:text-purple-400 uppercase">JazzCash</span>
                                    <span className="text-[8px] text-secondary dark:text-gray-500">Instant Pay</span>
                                </div>
                                <div className="bg-black/5 dark:bg-black/30 border border-black/5 dark:border-white/5 rounded-xl p-3 flex flex-col items-center gap-1 hover:border-green-500/30 transition-all cursor-default group/item">
                                    <span className="text-[10px] font-black text-green-600 dark:text-green-400 uppercase">EasyPaisa</span>
                                    <span className="text-[8px] text-secondary dark:text-gray-500">Safe & Fast</span>
                                </div>
                            </div>
                        </div>
            ...
            <button 
                ...
                className={`relative w-16 h-16 pointer-events-auto rounded-full flex items-center justify-center transition-all duration-500 transform hover:scale-110 active:scale-95 shadow-lg overflow-hidden ${isOpen ? 'bg-red-500/20 backdrop-blur-md border border-white/20 rotate-[135deg]' : 'bg-gradient-to-br from-green-500 to-emerald-600'}`}
            >
                {/* Background Pulse */}
