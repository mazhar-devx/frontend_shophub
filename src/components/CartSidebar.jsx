          ...
          <div className="h-full flex flex-col glass border-l border-white/10 shadow-2xl">
            <div className="flex-1 overflow-y-auto py-6 px-4 sm:px-6">
              <div className="flex items-start justify-between">
                <h2 className="text-xl font-bold text-primary dark:text-white">
                  Shopping Cart <span className="text-purple-600 dark:text-purple-400">({totalQuantity})</span>
                </h2>
                <button
                  type="button"
              ...
              <div className="mt-8">
                {cartItems.length === 0 ? (
                  <div className="text-center py-12">
                    ...
                    <h3 className="mt-2 text-lg font-medium text-primary dark:text-white">Your cart is empty</h3>
                    <p className="mt-1 text-sm text-secondary">Looks like you haven't added anything yet.</p>
                    <button onClick={toggleCart} className="mt-6 text-cyan-600 dark:text-cyan-400 hover:text-cyan-500 dark:hover:text-cyan-300 font-medium">Start Shopping &rarr;</button>
                  </div>
                ) : (
                          ...
                          <div className="ml-4 flex-1 flex flex-col">
                            <div>
                              <div className="flex justify-between text-base font-medium text-primary dark:text-white">
                                ...
                                <p className="text-cyan-600 dark:text-cyan-400">{formatPrice(item.price)}</p>
                              </div>
            ...
            {cartItems.length > 0 && (
              <div className="border-t border-white/10 py-6 px-4 sm:px-6 bg-black/40">
                <div className="flex justify-between text-base font-medium text-primary dark:text-white mb-4">
                  <p>Subtotal</p>
                  <p className="text-xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-cyan-600 to-purple-600 dark:from-cyan-400 dark:to-purple-500">{formatPrice(totalAmount)}</p>
                </div>
                <p className="mt-0.5 text-sm text-secondary mb-6">Shipping and taxes calculated at checkout.</p>
