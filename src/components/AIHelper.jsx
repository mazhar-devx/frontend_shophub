...
const ProductCard = memo(({ product, onClose }) => {
    ...
    return (
        <Link
            ...
            className="min-w-[230px] max-w-[230px] flex flex-col bg-gradient-to-br from-white/[0.08] to-white/[0.02] rounded-[2.5rem] border border-white/10 transition-all duration-500 group/card overflow-hidden relative shadow-2xl snap-start"
            style={{
                transform: isHovered ? `perspective(800px) rotateX(${tilt.x}deg) rotateY(${tilt.y}deg) scale(1.03)` : "perspective(800px) rotateX(0) rotateY(0) scale(1)",
        ...
        >
            {/* Holographic shimmer */}
            <div
                className="absolute inset-0 opacity-0 group-hover/card:opacity-100 transition-opacity duration-700 pointer-events-none z-20 rounded-[2.5rem]"
                style={{
            ...
            <div className="absolute top-4 right-4 z-10 flex flex-col gap-2 items-end">
                <div className="bg-cyan-500/90 backdrop-blur-md text-[7px] font-black px-2.5 py-1 rounded-full text-white uppercase tracking-[0.2em] shadow-lg border border-white/20 flex items-center gap-1">
            ...
            <div className="relative aspect-square overflow-hidden m-2 rounded-[2rem] bg-indigo-950/50">
                ...
                {/* Quick view badge */}
                <div className="absolute bottom-3 left-1/2 -translate-x-1/2 opacity-0 group-hover/card:opacity-100 translate-y-4 group-hover/card:translate-y-0 transition-all duration-300">
            ...
            <div className="px-5 pb-5 flex flex-col gap-1">
                ...
                <button
                    ...
                    className="mt-4 w-full py-3 bg-gradient-to-r from-cyan-600/50 to-purple-600/50 group-hover/card:from-cyan-500 group-hover/card:to-purple-600 text-[9px] font-black uppercase tracking-[0.2em] text-white rounded-2xl transition-all active:scale-95 shadow-xl border border-white/5"
                >
                    Purchase Now
                </button>
            </div>
...
const VideoCard = memo(({ video, onClose }) => (
    <Link
        ...
        onClick={onClose}
        className="min-w-[160px] max-w-[160px] aspect-[9/16] flex flex-col bg-black rounded-[2.5rem] border border-white/10 transition-all duration-500 group/vid overflow-hidden relative shadow-2xl snap-start hover:shadow-[0_0_40px_rgba(168,85,247,0.3)] hover:border-purple-500/30"
    >
        <img
        ...
        <div className="absolute inset-0 flex items-center justify-center">
            <div className="w-14 h-14 rounded-full bg-white/20 backdrop-blur-md flex items-center justify-center border border-white/40 group-hover/vid:scale-125 transition-all duration-500 shadow-2xl">
        ...
        <div className="absolute bottom-4 left-4 right-4">
            ...
            {video.views && <p className="text-[8px] text-gray-400 mt-1 font-bold">{video.views} views</p>}
        </div>
    </Link>
));
...
const TypingIndicator = memo(() => (
    <div className="flex justify-start animate-fade-in-up">
        <div className="bg-white/5 border border-white/10 p-5 rounded-[2rem] rounded-tl-none flex flex-col gap-3 shadow-2xl backdrop-blur-xl relative overflow-hidden">
            ...
            <div className="relative z-10 flex items-center gap-2">
                <div className="h-[2px] bg-cyan-500/30 rounded-full animate-[progress_2s_ease-in-out_infinite]" style={{ width: "60px" }}></div>
                <p className="text-[10px] text-gray-500 font-medium italic">Analyzing neural patterns...</p>
            </div>
        </div>
    </div>
));
...
export default function AIHelper() {
    ...
    const handleSendMessage = useCallback(async (e) => {
        ...
        try {
            ...
            if (data.status === "success") {
                ...
                // Advanced streaming with character-level animation
                let current = "";
    ...
    }, [isOpen, showSearch, showEmojiPicker]);
    ...
    const renderMessage = useCallback((msg) => {
        if (msg.role === "user") {
            return (
                <div className="space-y-1">
                    ...
                    <span className="text-[9px] text-white/40 font-bold">{msg.timestamp}</span>
                </div>
            );
        }
        ...
        return (
            <div className="space-y-4">
                <div className="space-y-1">
                    <div
                        className="text-[15px] leading-relaxed prose-invert"
                        dangerouslySetInnerHTML={{ __html: formatMarkdown(textContent) }}
                    />
                    <div className="flex items-center gap-3 pt-1">
                        <span className="text-[9px] text-gray-600 font-bold">{msg.timestamp}</span>
                        {msg.isError && <span className="text-[8px] text-red-400 font-bold uppercase tracking-widest">⚠ Error Response</span>}
                    </div>
                </div>

                {/* Product Cards */}
                {productsMatches.length > 0 && (
                    <div className="flex overflow-x-auto gap-4 pb-3 no-scrollbar -mx-2 px-2 snap-x snap-mandatory">
                        <div className="flex items-center gap-2 min-w-fit px-3">
                            <div className="w-1 h-8 bg-gradient-to-b from-cyan-500 to-purple-600 rounded-full"></div>
    ...
    const statusConfig = useMemo(() => ({
        connected: { color: "bg-green-500", text: "Neural Core Online", glow: "shadow-[0_0_8px_rgba(34,197,94,0.6)]" },
        connecting: { color: "bg-yellow-500", text: "Establishing Link...", glow: "animate-pulse" },
        processing: { color: "bg-cyan-500", text: "Processing Query...", glow: "animate-pulse shadow-[0_0_8px_rgba(34,211,238,0.6)]" },
        degraded: { color: "bg-orange-500", text: "Limited Connectivity", glow: "animate-pulse" },
        error: { color: "bg-red-500", text: "Link Disrupted", glow: "animate-pulse shadow-[0_0_8px_rgba(239,68,68,0.6)]" },
    }), []);
    ...
    return (
        <div className="fixed max-md:bottom-24 bottom-6 right-6 z-[1000] flex flex-col items-end w-auto pointer-events-none group/ai forced-dark">
            ...
            {/* ─── AI Chat Panel ─── */}
            <div
                className={`w-[95vw] sm:w-[500px] mb-6 pointer-events-auto transition-all duration-700 ${isOpen && !isMinimized
                    ? "translate-y-0 opacity-100 scale-100"
                    : "translate-y-20 opacity-0 scale-95 pointer-events-none"
                    }`}
                style={{ transitionTimingFunction: "cubic-bezier(0.16, 1, 0.3, 1)" }}
            >
                <div className="bg-[#050508] border border-white/[0.08] rounded-[2.5rem] shadow-[0_50px_150px_-20px_rgba(0,0,0,1),0_0_80px_-20px_rgba(34,211,238,0.08)] overflow-hidden flex flex-col h-[85vh] sm:h-[720px] relative">
                    ...
                    {/* Ambient Glow */}
                    <div className="absolute top-0 left-1/4 w-1/2 h-1/3 bg-cyan-500/[0.03] rounded-full blur-[100px] pointer-events-none" />
                    <div className="absolute bottom-0 right-1/4 w-1/2 h-1/3 bg-purple-500/[0.03] rounded-full blur-[100px] pointer-events-none" />
                        ...
                        <div className="p-5 pb-4 flex items-center justify-between">
                            <div className="flex items-center gap-4">
                                <div className="relative group">
                                    <div className="absolute -inset-1.5 bg-gradient-to-r from-cyan-400 via-indigo-500 to-purple-600 rounded-2xl blur-lg opacity-60 group-hover:opacity-100 transition duration-1000" />
                                    <div className="relative w-13 h-13 rounded-2xl bg-black border border-white/20 flex items-center justify-center shadow-2xl overflow-hidden" style={{ width: 52, height: 52 }}>
                                        <svg className="w-8 h-8 text-cyan-400 drop-shadow-[0_0_10px_rgba(34,211,238,0.9)]" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
                                            <path d="M12 2a10 10 0 0 0-10 10c0 5.52 4.48 10 10 10s10-4.48 10-10A10 10 0 0 0 12 2z" />
                                            <path d="M12 6v6l4 2" />
                                            <circle cx="12" cy="12" r="3" fill="currentColor" fillOpacity="0.2" />
                                        </svg>
                                        <div className="absolute -bottom-0.5 -right-0.5 w-4 h-4 bg-green-500 border-2 border-black rounded-full animate-pulse shadow-[0_0_12px_rgba(34,197,94,0.6)]"></div>
                                    </div>
                                </div>
                                <div>
                                    <h3 className="text-white font-black text-lg leading-tight tracking-tighter uppercase italic bg-clip-text text-transparent bg-gradient-to-r from-white via-cyan-200 to-indigo-300">
                                        HA DEEP BRAIN
                                    </h3>
                                    <div className="flex items-center gap-2 mt-0.5">
                                        <div className={`w-1.5 h-1.5 ${statusConfig[connectionStatus].color} rounded-full ${statusConfig[connectionStatus].glow}`} />
                                        <span className="text-[8px] text-gray-400 font-black uppercase tracking-[0.25em]">{statusConfig[connectionStatus].text}</span>
                                        <span className="text-[7px] text-gray-600 font-bold">•</span>
                                        <span className="text-[8px] text-gray-600 font-black uppercase tracking-widest">{NEURAL_VERSION}</span>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div className="flex gap-1.5">
                                ...
                                <button
                                    ...
                                    className="p-2.5 bg-white/[0.04] hover:bg-white/[0.08] rounded-xl border border-white/[0.06] transition-all active:scale-90"
                                ...
                                <button
                                    ...
                                    className="p-2.5 bg-white/[0.04] hover:bg-red-500/20 rounded-xl border border-white/[0.06] transition-all active:scale-90"
                                    title="Format Memory"
                                >
                            ...
                            <div className="px-5 pb-4 animate-fade-in-up">
                                <div className="relative">
                                    ...
                                    <input
                                        ...
                                        className="w-full bg-white/[0.06] border border-white/[0.08] rounded-xl pl-11 pr-4 py-2.5 text-sm text-white focus:border-cyan-500/50 outline-none transition-all placeholder-gray-600 input-glow"
                                        autoFocus
                                    />
                                    {searchQuery && (
                                ...
                                {searchQuery && (
                                    <p className="text-[9px] text-gray-500 font-bold mt-2 px-1">
                    ...
                    <div
                        ...
                        className="flex-1 overflow-y-auto p-5 space-y-5 custom-scrollbar bg-transparent relative z-10"
                    >
                        {/* Contact Section */}
                        <div className="animate-fade-in-up">
                            <div className="bg-gradient-to-br from-green-600/[0.08] to-emerald-600/[0.04] rounded-[2rem] border border-green-500/[0.12] p-5 relative overflow-hidden group/contact">
                                <div className="absolute top-0 left-0 w-1 h-full bg-gradient-to-b from-green-400 to-emerald-600 rounded-full"></div>
                                <div className="flex items-center gap-4 mb-4">
                                    <div className="w-11 h-11 rounded-2xl bg-gradient-to-br from-green-500 to-emerald-700 flex items-center justify-center text-xl shadow-xl border border-green-400/30">👨‍💼</div>
                                    <div>
                                        <h4 className="text-white font-black text-sm uppercase tracking-tight">Direct Support — Asad</h4>
                                        <p className="text-[8px] text-green-400 font-bold uppercase tracking-[0.2em] flex items-center gap-1.5">
                                ...
                                <div className="grid grid-cols-2 gap-2.5 mb-4">
                                    <div className="bg-white/[0.04] border border-white/[0.05] rounded-xl p-2.5 flex flex-col items-center gap-0.5 hover:bg-white/[0.06] transition-colors">
                                        <span className="text-[10px] font-black text-purple-400 uppercase">JazzCash</span>
                                        <span className="text-[7px] text-gray-600 font-bold uppercase tracking-widest">Secure Pay</span>
                                    </div>
                                    <div className="bg-white/[0.04] border border-white/[0.05] rounded-xl p-2.5 flex flex-col items-center gap-0.5 hover:bg-white/[0.06] transition-colors">
                                        <span className="text-[10px] font-black text-green-400 uppercase">EasyPaisa</span>
                                        <span className="text-[7px] text-gray-600 font-bold uppercase tracking-widest">Instant Pay</span>
                                    </div>
                                </div>
                                <button
                                    onClick={() => window.open(`https://wa.me/${WHATSAPP_NUMBER}?text=${encodeURIComponent(WHATSAPP_WELCOME)}`, "_blank")}
                                    className="w-full bg-gradient-to-br from-green-500 to-emerald-700 text-white font-black text-[11px] py-3.5 rounded-2xl shadow-xl hover:shadow-green-500/25 transition-all transform hover:-translate-y-0.5 active:scale-[0.98] flex items-center justify-center gap-2.5 border border-green-400/20"
                                >
                                    ...
                                    <span className="uppercase tracking-[0.2em]">WhatsApp Admin</span>
                                </button>
                            </div>
                        </div>
                        ...
                        {filteredMessages.map((msg, idx) => (
                            ...
                            >
                                <div
                                    className={`message-bubble relative max-w-[88%] p-4 rounded-[1.8rem] shadow-2xl transition-all duration-300 ${msg.role === "user"
                                        ? "bg-gradient-to-br from-cyan-600 to-indigo-700 text-white rounded-tr-none border border-white/10"
                                        : `bg-white/[0.04] border border-white/[0.08] text-cyan-50 rounded-tl-none backdrop-blur-xl ${msg.isError ? "border-red-500/20 bg-red-500/[0.04]" : ""}`
                                        }`}
                                >
                                    {renderMessage(msg)}

                                    {/* Message Actions (hover) */}
                                    <div className="absolute -bottom-2 right-2 opacity-0 group-hover/msg:opacity-100 transition-all duration-200 flex gap-1 translate-y-1 group-hover/msg:translate-y-0">
                                        <button
                                    ...
                                    {(msg.reactions || []).length > 0 && (
                                        <div className="flex gap-1 mt-2">
                                            {msg.reactions.map((r, ri) => (
                                                <span key={ri} className="bg-white/10 text-[10px] px-1.5 py-0.5 rounded-full">{r.emoji}</span>
                                            ))}
                                        </div>
                                    )}
                                </div>
                        ...
                        {streamingMessage && (
                            <div className="flex justify-start animate-fade-in">
                                <div className="bg-white/[0.04] border border-white/[0.08] p-4 rounded-[1.8rem] rounded-tl-none text-cyan-50 shadow-2xl backdrop-blur-xl max-w-[88%]">
                                    <div
                                        className="text-[15px] leading-relaxed"
                                        dangerouslySetInnerHTML={{ __html: formatMarkdown(streamingMessage) }}
                                    />
                                    <span className="inline-block w-2 h-4 bg-cyan-400 animate-pulse ml-1 align-middle rounded-sm"></span>
                                </div>
                            </div>
                        )}
                    ...
                    {/* ─── Quick Suggestions ─── */}
                    <div className="px-5 py-2.5 bg-[#050508]/80 backdrop-blur-md flex gap-2 overflow-x-auto no-scrollbar pointer-events-auto relative z-10 border-t border-white/[0.04]">
                        {currentSuggestions.map((text) => (
                            <button
                                ...
                                className="whitespace-nowrap px-3.5 py-1.5 bg-white/[0.04] hover:bg-cyan-500/[0.12] border border-white/[0.08] hover:border-cyan-500/20 rounded-full text-[9px] font-black text-gray-400 hover:text-cyan-400 uppercase tracking-[0.15em] transition-all active:scale-95"
                            >
                                {text}
                            </button>
                        ))}
                    ...
                    <form
                        ...
                        className="px-5 py-4 bg-[#08080c]/90 backdrop-blur-2xl border-t border-white/[0.06] flex gap-2.5 pointer-events-auto relative z-10"
                    >
                        {/* Voice Input */}
                        <button
                            type="button"
                            onClick={toggleVoice}
                            className={`w-12 h-12 rounded-2xl flex items-center justify-center transition-all active:scale-90 border ${isListening
                        ...
                        <input
                            ...
                            className="flex-1 bg-white/[0.04] border border-white/[0.06] rounded-2xl px-5 py-3.5 text-sm text-white focus:border-cyan-500/40 outline-none transition-all placeholder-gray-600 input-glow"
                            disabled={isLoading}
                        />
                        ...
                        <button
                            ...
                            className="w-12 h-12 rounded-2xl bg-gradient-to-br from-cyan-600 via-indigo-600 to-purple-700 flex items-center justify-center text-white hover:scale-105 transition-all shadow-[0_0_25px_rgba(124,58,237,0.25)] active:scale-95 disabled:opacity-30 disabled:hover:scale-100 group/btn"
                        >
                            <svg className="w-5 h-5 transform rotate-90 group-hover/btn:translate-x-0.5 transition-transform" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    ...
                    {/* Footer */}
                    <div className="px-5 pb-3 pt-0 flex items-center justify-between relative z-10">
                        <span className="text-[7px] text-gray-700 font-bold uppercase tracking-widest">HA Deep Brain {NEURAL_VERSION}</span>
                        <span className="text-[7px] text-gray-700 font-bold uppercase tracking-widest">{messages.length} messages</span>
                    </div>
                </div>
            </div>
            ...
            {/* ─── Minimized Bar ─── */}
            <div
                className={`w-[95vw] sm:w-[500px] mb-6 pointer-events-auto transition-all duration-500 ${isOpen && isMinimized
                    ? "translate-y-0 opacity-100 scale-100"
            ...
            >
                <button
                    ...
                    className="w-full bg-[#050508]/95 backdrop-blur-2xl border border-white/[0.08] rounded-2xl px-5 py-3.5 flex items-center justify-between shadow-2xl hover:border-cyan-500/20 transition-all"
                >
                    <div className="flex items-center gap-3">
                        <div className="w-2 h-2 bg-green-500 rounded-full animate-pulse" />
                        <span className="text-[11px] text-white font-black uppercase tracking-wider">Deep Brain Active</span>
                    </div>
                    <div className="flex items-center gap-3">
                        {streamingMessage && <span className="text-[9px] text-cyan-400 font-bold animate-pulse">Typing...</span>}
                        <svg className="w-4 h-4 text-gray-500" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 15l7-7 7 7" /></svg>
                    </div>
                </button>
            </div>
            ...
            <button
                ...
                className={`pointer-events-auto group relative flex items-center justify-center transition-all duration-700 transform hover:scale-110 active:scale-90 ${isOpen ? "rotate-0" : ""}`}
            ...
            >
                ...
                {/* Orbital Rings */}
                <div className={`absolute w-28 h-28 rounded-full border border-cyan-500/20 transition-all duration-1000 ${isOpen ? "scale-0 opacity-0" : "animate-[orbit_8s_linear_infinite]"}`}>
                ...
                <div className={`absolute w-24 h-24 rounded-full border border-purple-500/15 transition-all duration-1000 ${isOpen ? "scale-0 opacity-0" : "animate-[orbit_6s_linear_infinite_reverse]"}`}>
                ...
                {/* Main Sphere */}
                <div
                    className={`relative w-20 h-20 rounded-full transition-all duration-500 flex items-center justify-center overflow-hidden ${isOpen
                ...
                >
                    ...
                    {isOpen ? (
                        <svg className="w-7 h-7 text-white/80 relative z-10" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M19 9l-7 7-7-7" />
                        </svg>
                    ) : (
                        <div className="relative z-10 flex flex-col items-center">
                            <div className="relative">
                                <span className="text-3xl filter drop-shadow-[0_0_12px_rgba(255,255,255,0.4)]">🧠</span>
                                <div className="absolute -top-1 -right-1 w-3 h-3 bg-cyan-400 rounded-full animate-ping" />
                                <div className="absolute -top-1 -right-1 w-3 h-3 bg-cyan-400 rounded-full shadow-[0_0_12px_#22d3ee]" />
                            </div>
                            <span className="text-[7px] font-black tracking-[0.2em] text-cyan-400/80 uppercase leading-none mt-0.5">Deep Brain</span>
                        </div>
                    )}
                </div>
                ...
                {unreadCount > 0 && !isOpen && (
                    <div className="absolute -top-1 -right-1 min-w-[20px] h-[20px] bg-gradient-to-r from-pink-500 to-red-500 rounded-full flex items-center justify-center text-[9px] font-black text-white shadow-[0_0_15px_rgba(236,72,153,0.5)] border-2 border-[#050508] animate-bounce">
