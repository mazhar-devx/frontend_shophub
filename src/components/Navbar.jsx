        ...
        {isMenuOpen && (
          <div className="fixed inset-0 z-50 md:hidden">
            ...
            <motion.div 
              ...
              exit={{ x: '100%' }}
              transition={{ type: "spring", stiffness: 300, damping: 30 }}
              className="absolute top-0 right-0 w-[85%] max-w-sm h-full bg-[#050505] border-l border-white/10 shadow-2xl flex flex-col"
            >
              <div className="flex items-center justify-between p-6 border-b border-white/5">
                 <span className="text-xl font-black text-white uppercase tracking-widest">Navigation</span>
                 <button onClick={toggleMenu} className="w-10 h-10 flex items-center justify-center rounded-full bg-white/5 text-white">
                   <X className="w-5 h-5" />
                 </button>
              </div>
              ...
              <div className="p-6 border-t border-white/5">
                {!isAuthenticated ? (
                   <div className="grid grid-cols-2 gap-4">
                      <Link to="/login" onClick={toggleMenu} className="py-4 text-center rounded-2xl border border-white/10 text-white font-bold hover:bg-white/5">Login</Link>
                      <Link to="/register" onClick={toggleMenu} className="py-4 text-center rounded-2xl bg-cyan-600 text-white font-bold shadow-lg shadow-cyan-500/20">Sign Up</Link>
                   </div>
                ) : (
                   <div className="bg-white/5 rounded-3xl p-6">
                      <div className="flex items-center gap-4 mb-6">
                         <img src={getProductImageUrl(user?.photo) || DEFAULT_AVATAR} className="w-14 h-14 rounded-2xl border border-white/10 object-cover" />
                         <div>
                            <p className="font-black text-white">{user?.role === 'admin' ? (user?.vendorName || user?.name) : user?.name}</p>
                            <p className="text-xs text-gray-500">{user?.email}</p>
                         </div>
                      </div>
                      <button onClick={() => { handleLogout(); toggleMenu(); }} className="w-full py-4 rounded-2xl bg-red-500/10 text-red-400 font-bold hover:bg-red-500/20 flex items-center justify-center gap-3">
